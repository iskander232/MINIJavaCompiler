...
void TreeBuilder::Visit(ArrayLvalue *array_lvalue) {
  tree_.GetCurrentLayer()->DeclareVariable(Symbol(array_lvalue->GetName()));
}

void TreeBuilder::Visit(SimpleLvalue *simple_lvalue) {
  tree_.GetCurrentLayer()->DeclareVariable(Symbol(simple_lvalue->GetName()));
}
...
void TreeBuilder::Visit(VarDecl *var_decl) {
  ...
  tree_.GetCurrentLayer()->Put(Symbol(var_decl->GetName()), var_decl->GetType());
}
