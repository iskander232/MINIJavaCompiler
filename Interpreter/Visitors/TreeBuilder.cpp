...
std::map<std::pair<Symbol, Symbol>, std::shared_ptr<FunctionObject> > TreeBuilder::GetFunctions() {
  return functions_;
}
...
void TreeBuilder::Visit(MethodDecl *method_decl) {
  ...
  auto symbol = std::make_pair(Symbol(my_class->name_), Symbol(method_decl->GetName()));
  ...
  current_layer_->DeclareFunction(symbol, func);
  ...
  tree_.AddMapping(symbol, new_layer);
...
void TreeBuilder::Visit(Main *main) {
  ...
  auto symbol = std::make_pair(Symbol(main->GetName()), Symbol("main"));
